...
export default function TransactionsScreen({ navigation }: TransactionsScreenProps): React.JSX.Element {
    ...
    const fetchTransactions = useCallback(async (showLoading = true): Promise<void> => {
        try {
    ...
    const renderTransaction = useCallback(({ item }: { item: Transaction }): React.JSX.Element => {
        ...
        <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
            ...
            <View className="px-5 pt-4 pb-4 flex-row items-center justify-between">
                <View>
                    ...
                    <Text className="text-gray-500 mt-1">{transactions.length} transactions</Text>
                </View>
                <TouchableOpacity
        ...
        <Card variant="elevated" padding="md">
            <View className="flex-row items-center">
                <View className="w-12 h-12 rounded-full bg-gray-100 items-center justify-center mr-4">
                    <Text className="text-xl">{getCategoryIcon(transaction.category)}</Text>
                </View>
                ...
                <View className="items-end">
                    ...
                    <View
                        className={`mt-1 px-2 py-0.5 rounded-full ${isIncome ? 'bg-green-100' : 'bg-red-100'
                            }`}
                    >
                        <Text
