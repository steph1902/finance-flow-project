...
export class DemoDataService {
  ...
  static generateTransactions(
    ...
    const incomeRatio = 0.15 + Math.random() * 0.2;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isIncome) {
        ...
        if (isFixed) {
          ...
          category =
            categories.fixed[
              Math.floor(Math.random() * categories.fixed.length)
            ];
          ...
          category =
            categories.discretionary[
              Math.floor(Math.random() * categories.discretionary.length)
            ];
          description =
            category === "Dining"
              ? "Restaurant / Cafe"
   ...
   */
  static generateExperimentResults(
    experimentId: string,
    userId: string,
  ): Prisma.AIExperimentResultCreateManyInput[] {
    ...
    for (let i = 0; i < 509; i++) {
      ...
      results.push({
        ...
        requestData: {
          prompt: "Analyze my cashflow risks using Big 4 methodology",
        },
        responseData: {
          analysis:
            "Cashflow risk detected: high variance in discretionary spending.",
          score: 85,
        },
      });
    }
