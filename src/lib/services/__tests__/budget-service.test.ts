...
describe("Budget Service", () => {
  ...
  describe("getBudgets", () => {
    it("should return budgets with calculated progress", async () => {
      ...
      (prisma.transaction.groupBy as unknown as any).mockResolvedValue(
        mockSpending,
      );
    ...
    it("should handle errors", async () => {
      (prisma.budget.findMany as unknown as any).mockRejectedValue(
        new Error("DB Error"),
      );
      await expect(budgetService.getBudgets("u1", 1, 2024)).rejects.toThrow(
  ...
  describe("createBudget", () => {
    ...
    it("should handle other creation errors", async () => {
      (prisma.budget.create as unknown as any).mockRejectedValue(
        new Error("DB Error"),
      );
      await expect(
        budgetService.createBudget("u1", {
  ...
  describe("updateBudget", () => {
    it("should update budget", async () => {
      (prisma.budget.findFirst as unknown as any).mockResolvedValue({
        id: "b1",
      });
      (prisma.budget.update as unknown as any).mockResolvedValue({
        id: "b1",
        amount: new Prisma.Decimal(600),
      });

      const result = await budgetService.updateBudget("u1", "b1", {
        amount: 600,
      });
    ...
    it("should throw if not found", async () => {
      ...
      await expect(budgetService.updateBudget("u1", "b1", {})).rejects.toThrow(
        "Budget not found",
      );
    });
  });
  ...
  describe("deleteBudget", () => {
    ...
    it("should throw if not found", async () => {
      (prisma.budget.findFirst as unknown as any).mockResolvedValue(null);
      await expect(budgetService.deleteBudget("u1", "b1")).rejects.toThrow(
        "Budget not found",
      );
    });
  });
