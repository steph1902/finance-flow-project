...
describe('Transaction Service', () => {
        ...
        it('should apply filters', async () => {
            ...
            expect(prisma.transaction.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: expect.objectContaining({
                    type: 'EXPENSE',
                    category: 'Food',
                    OR: expect.arrayContaining([{ description: { contains: 'Coffee', mode: 'insensitive' } }]),
                }),
            }));
        });

        it('should handle errors', async () => {
            (prisma.transaction.findMany as unknown as any).mockRejectedValue(new Error('DB Error'));
            await expect(transactionService.getTransactions('user-1', {
                page: 1,
                limit: 10,
                sort: 'date',
                order: 'desc'
    ...
    describe('createTransaction', () => {
        it('should create a transaction', async () => {
            ...
            const result = await transactionService.createTransaction('user-1', input);
        ...
        it('should handle creation errors', async () => {
            (prisma.transaction.create as unknown as any).mockRejectedValue(new Error('DB Error'));
            await expect(transactionService.createTransaction('user-1', {
                amount: 100, date: new Date(), type: 'EXPENSE', category: 'Food'
    ...
    describe('getTransaction', () => {
        it('should return transaction if found', async () => {
            ...
            (prisma.transaction.findFirst as unknown as any).mockResolvedValue(mockTx);
    ...
    describe('updateTransaction', () => {
        it('should update transaction if found', async () => {
            (prisma.transaction.findFirst as unknown as any).mockResolvedValue({ id: 'tx-1' });
            (prisma.transaction.update as unknown as any).mockResolvedValue({ id: 'tx-1', amount: 200 });

            const result = await transactionService.updateTransaction('user-1', 'tx-1', { amount: 200 });
        ...
        it('should throw if not found', async () => {
            (prisma.transaction.findFirst as unknown as any).mockResolvedValue(null);
            await expect(transactionService.updateTransaction('user-1', 'tx-1', {})).rejects.toThrow('Transaction not found');
        });
    });
    ...
    describe('deleteTransaction', () => {
        ...
        it('should throw if not found', async () => {
            (prisma.transaction.findFirst as unknown as any).mockResolvedValue(null);
            await expect(transactionService.deleteTransaction('user-1', 'tx-1')).rejects.toThrow('Transaction not found');
        });
    });
});
