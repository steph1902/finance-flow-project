...
describe('Goal Service', () => {
    ...
    describe('createGoal', () => {
        it('should create a goal and milestones', async () => {
            ...
            const mockGoal = { id: 'goal-1', ...input, milestones: [], contributions: [] };
        ...
        it('should handle creation errors', async () => {
            (prisma.goal.create as unknown as any).mockRejectedValue(new Error('DB Error'));
            await expect(goalService.createGoal({
                userId: 'u1', name: 'G', targetAmount: 100
    ...
    describe('updateGoal', () => {
        it('should update goal if found and authorized', async () => {
            (prisma.goal.updateMany as unknown as any).mockResolvedValue({ count: 1 });

            await goalService.updateGoal('goal-1', 'user-1', { name: 'Updated Name' });
        ...
        it('should throw if goal not found', async () => {
            (prisma.goal.updateMany as unknown as any).mockResolvedValue({ count: 0 });
            await expect(goalService.updateGoal('goal-1', 'user-1', { name: 'U' })).rejects.toThrow('Failed to update goal');
    ...
    describe('addContribution', () => {
        it('should add contribution and update progress', async () => {
            const mockGoal = {
                ...
                milestones: [
                    { id: 'm1', amount: new Prisma.Decimal(1000), achievedAt: null, description: '100% milestone' }
                ],
            };
            ...
            await goalService.addContribution({ goalId: 'goal-1', amount: 500 }, 'user-1');

            // Check contribution creation
            expect(prisma.goalContribution.create).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(prisma.goalMilestone.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'm1' },
