...
describe("Investment Service", () => {
  ...
  describe("createInvestment", () => {
    it("should create a new investment with initial BUY transaction", async () => {
      ...
      (prisma.investment.create as unknown as any).mockResolvedValue(
        mockInvestment,
      );
  ...
  describe("recordInvestmentTransaction", () => {
    it("should handle BUY transaction and update investment", async () => {
      ...
      (prisma.investment.findUnique as unknown as any).mockResolvedValue(
        mockInvestment,
      );
      (prisma.investmentTransaction.create as unknown as any).mockResolvedValue(
        {
      ...
      const result = await investmentService.recordInvestmentTransaction(
        "inv-123",
        {
  ...
  describe("getPortfolioSummary", () => {
    it("should calculate portfolio totals and breakdown", async () => {
      ...
      expect(
        Object.keys(result.byType).filter(
          (k) => result.byType[k as keyof typeof result.byType].count > 0,
        ),
      ).toHaveLength(2);
    });

    it("should throw error if summary calculation fails", async () => {
      (prisma.investment.findMany as unknown as any).mockRejectedValue(
        new Error("DB Error"),
      );
      await expect(
        investmentService.getPortfolioSummary("user-123"),
      ).rejects.toThrow("Failed to calculate portfolio summary");
    });
  });
  ...
  describe("deleteInvestment", () => {
    it("should delete investment if user owns it", async () => {
      ...
      (prisma.investment.findFirst as unknown as any).mockResolvedValue(
        mockInvestment,
      );
  ...
  describe("updateInvestmentValue", () => {
    it("should update investment value and timestamp", async () => {
      ...
      const result = await investmentService.updateInvestmentValue(
        "inv-123",
        2000,
      );
    ...
    it("should throw error if update fails", async () => {
      (prisma.investment.update as unknown as any).mockRejectedValue(
  ...
  describe("getInvestmentTransactions", () => {
    it("should return transactions for an investment", async () => {
      ...
      (
        prisma.investmentTransaction.findMany as unknown as any
      ).mockResolvedValue(mockTx);

      const result =
        await investmentService.getInvestmentTransactions("inv-123");
    ...
    it("should handle errors", async () => {
      ...
      await expect(
        investmentService.getInvestmentTransactions("inv-123"),
  ...
  describe("getPortfolioPerformance", () => {
    it("should calculate performance data points", async () => {
      const mockInvestments = [{ currentValue: 1000 }, { currentValue: 2000 }];
      (prisma.investment.findMany as unknown as any).mockResolvedValue(
        mockInvestments,
      );

      const result = await investmentService.getPortfolioPerformance(
        "user-123",
        30,
      );
    ...
    it("should handle performance calculation errors", async () => {
      ...
      await expect(
        investmentService.getPortfolioPerformance("user-123"),
      ).rejects.toThrow("Failed to calculate performance");
    });
  });
});
