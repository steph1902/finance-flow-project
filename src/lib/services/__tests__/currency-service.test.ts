...
describe('Currency Service', () => {
    ...
    describe('formatCurrency', () => {
        it('should format USD correctly', () => {
            expect(currencyService.formatCurrency(1000, 'USD')).toContain('$1,000.00');
        });
    ...
    describe('updateUserCurrency', () => {
        it('should update user currency', async () => {
            (prisma.user.update as unknown as any).mockResolvedValue({ id: 'u1', preferredCurrency: 'EUR' });
        ...
        it('should throw on error', async () => {
            (prisma.user.update as unknown as any).mockRejectedValue(new Error('DB Error'));

            await expect(currencyService.updateUserCurrency('u1', 'EUR')).rejects.toThrow('Failed to update currency preference');
        });
    });
