...
export const DEMO_AI_INSIGHTS = [
  {
    type: "spending_alert",
    message:
      "You're spending $240/mo on coffee - that's a vacation fund! ☕→✈️",
    severity: "info",
  },
  {
  ...
  },
  {
    type: "budget_tip",
    message:
      'Try the "no eating out" challenge this week. You could save $150! 💪',
    severity: "suggestion",
  },
  {
    type: "subscription_audit",
    message:
      "You have 5 active subscriptions totaling $130/mo. Still using all of them? 🤔",
    severity: "warning",
  },
];
...
export function calculateDemoStats() {
  const totalIncome = DEMO_TRANSACTIONS.filter((t) => t.amount > 0).reduce(
    (sum, t) => sum + t.amount,
    0,
  );

  const totalExpenses = Math.abs(
    DEMO_TRANSACTIONS.filter((t) => t.amount < 0).reduce(
      (sum, t) => sum + t.amount,
      0,
    ),
  );
  ...
  return {
    ...
    averageTransaction:
      totalExpenses / DEMO_TRANSACTIONS.filter((t) => t.amount < 0).length,
  };
}
