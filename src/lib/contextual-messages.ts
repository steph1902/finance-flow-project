...
export function getTimeBasedGreeting(): string {
  ...
  if (hour < 6)
    return "Burning the midnight oil? Your finances can wait till morning 😴";
  if (hour < 12)
    return "Good morning! Let's make your money work smarter today ☀️";
...
export function getSavingsRateMessage(savingsRate: number): string {
  ...
  if (savingsRate >= 20)
    return "Excellent discipline! You're building wealth 💎";
...
export function getTransactionMilestone(count: number): string | null {
  if (count === 1)
    return "🎉 First transaction! Your financial journey begins!";
  if (count === 10)
    return "💪 10 transactions tracked! You're building great habits!";
