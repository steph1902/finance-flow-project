...
describe('budget-optimizer-service', () => {
  ...
  describe('optimizeBudgets', () => {
    ...
    it('should generate optimization suggestions using AI', async () => {
      const budgets = [{ category: 'Food', amount: 100, id: '1', month: 1, year: 2024 }] as any[];
      const transactions = [{ category: 'Food', amount: 150, date: new Date() }] as any[];
    ...
    it('should throw error on AI failure', async () => {
      ...
      await expect(service.optimizeBudgets({
        budgets: [],
        transactions: [],
        months: 1,
        userId: 'user1'
