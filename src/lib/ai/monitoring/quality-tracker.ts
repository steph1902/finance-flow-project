...
export class QualityTracker {
  ...
  async aggregateDailyMetrics(
    ...
    if (analyses.length === 0) {
      console.log(
        `No analyses found for ${variant} on ${date.toISOString().split("T")[0]}`,
      );
      return;
    }
    ...
    const avgSpecificity =
      specificities.length > 0
        ? specificities.reduce(
            (sum, a) => sum + Number(a.specificityScore!),
            0,
          ) / specificities.length
        : 0;

    const confidences = analyses.filter((a) => a.confidenceScore !== null);
    const avgConfidence =
      confidences.length > 0
        ? confidences.reduce((sum, a) => sum + Number(a.confidenceScore!), 0) /
          confidences.length
        : 0;
    ...
    const avgPromptTokens =
      withTokens.length > 0
        ? withTokens.reduce((sum, a) => sum + (a.promptTokens || 0), 0) /
          withTokens.length
        : 0;

    const avgCompletionTokens =
      withTokens.length > 0
        ? withTokens.reduce((sum, a) => sum + (a.completionTokens || 0), 0) /
          withTokens.length
        : 0;
    ...
    console.log(
      `✅ Aggregated metrics for ${variant} on ${date.toISOString().split("T")[0]}`,
    );
  }
  ...
  async detectDrift(
    ...
    const severity =
      driftMetrics.length >= 3
        ? "high"
        : driftMetrics.length >= 2
          ? "medium"
          : "low";
