...
export class FinancialDataAggregator {
  ...
  async aggregateData(userId: string): Promise<FinancialMetrics> {
    ...
    const discretionaryVariability =
      this.calculateDiscretionaryVariability(monthlyData);

    // Calculate burn rate and safety margin
    const totalExpenses90d = monthlyData.reduce(
    ...
    const bufferMultiple =
      monthlyExpenseAvg > 0 ? cashBuffer / monthlyExpenseAvg : 0;
    const safetyMargin = (bufferMultiple / 3) * 100; // % of 3x target

    // Calculate spending breakdown
    const discretionarySpending =
      this.calculateDiscretionarySpending(transactions);
  ...
  async getOrCreateSnapshot(userId: string): Promise<FinancialMetrics> {
    ...
    if (cached) {
      ...
      return {
        ...
        incomeStability: cached.incomeStability as
          | "stable"
          | "variable"
          | "volatile",
  ...
  private calculateDiscretionaryVariability(
    monthlyData: MonthlyBreakdown[],
  ): number {
    if (monthlyData.length < 2) return 0;
    ...
    const avg =
      discretionaryAmounts.reduce((a, b) => a + b, 0) /
      discretionaryAmounts.length;

    if (avg === 0) return 0;

    // Calculate standard deviation as % of mean
    const variance =
      discretionaryAmounts.reduce(
  ...
  private determineIncomeStability(
    monthlyData: MonthlyBreakdown[],
    ...
    const variance =
      incomes.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) /
      incomes.length;
  ...
  private calculateDiscretionarySpending(transactions: Transaction[]): number {
    return transactions
      .filter(
        (t) =>
          t.type === "EXPENSE" && DISCRETIONARY_CATEGORIES.includes(t.category),
      )
