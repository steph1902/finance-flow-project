...
> {
  ...
  async start(): Promise<void> {
    ...
    this.running = true;
    logInfo(
      `🤖 Agent ${this.name} started (interval: ${this.runIntervalMs}ms)`,
    );
   ...
   */
  protected abstract logDecision(
    log: AgentDecisionLog<TState, TInsight, TAction>,
  ): Promise<void>;
  ...
  protected generateReasoningSummary(
    ...
    return (
      `Detected ${highSeverityInsights.length} high-priority patterns. ` +
