...
> {
  ...
  protected async observe(): Promise<BudgetState> {
    ...
    const [budgets, transactions, recurring, historicalPatterns] =
      await Promise.all([
  ...
  protected async analyze(state: BudgetState): Promise<AgentInsight[]> {
    ...
    const transactionsByUserCategory =
      this.groupTransactionsByUserCategory(transactions);

    // Analyze each budget
    for (const budget of budgets) {
      const userCategoryKey = `${budget.userId}_${budget.category}`;
      const categoryTransactions =
        transactionsByUserCategory.get(userCategoryKey) || [];
      ...
      const categoryRecurring = recurring
        .filter(
          (r) => r.userId === budget.userId && r.category === budget.category,
        )
      ...
      const expectedSpending =
        (Number(budget.amount) / daysInMonth) * daysElapsed;
      ...
      if (projectedEndOfMonthSpending > Number(budget.amount)) {
        const overrunAmount =
          projectedEndOfMonthSpending - Number(budget.amount);
      ...
      if (paceRatio > 1.5 && daysElapsed > 7) {
        // Spending too fast, and we're past the first week
        insights.push({
      ...
      if (
        historicalAverage &&
        currentSpent > historicalAverage * 1.3 &&
        daysElapsed > 14
      ) {
        insights.push({
  ...
  protected async learn(cycle: {
    ...
    const learningMetrics = {
      ...
      highSeverityCount: cycle.insights.filter(
  ...
  protected async logDecision(
    log: AgentDecisionLog<BudgetState, AgentInsight, AgentAction>,
  ): Promise<void> {
    try {
      ...
      await prisma.agentDecisionLog.create({
        data: {
          ...
          highSeverityCount: log.insights.filter(
            (i) => i.severity === "high" || i.severity === "critical",
          ).length,
  ...
  private groupTransactionsByUserCategory(
    transactions: Transaction[],
  ...
  private async getHistoricalPatterns(
    ...
    for (const pattern of patterns.values()) {
      pattern.averagePerTransaction =
        pattern.totalSpent / pattern.transactionCount;
    }
  ...
  private getHistoricalAverage(
    patterns: MonthlySpendingPattern[],
    category: string,
  ): number | null {
    ...
    const totalSpent = categoryPatterns.reduce(
