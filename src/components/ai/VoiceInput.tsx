...
function parseVoiceCommand(transcript: string): VoiceCommand {
  ...
  const addMatch = lower.match(
    /(?:add|spent|paid)\s+(?:\$)?(\d+(?:\.\d+)?)\s+(?:dollars?\s+)?(?:on\s+)?(.+)/i,
  );
  ...
  const navMatch = lower.match(
    /(?:go to|open|show)\s+(dashboard|budgets?|goals?|settings?|transactions?)/i,
  );
...
function inferCategory(description: string): string {
  ...
  if (lower.match(/coffee|restaurant|lunch|dinner|food|eating/))
    return "Food & Dining";
  if (lower.match(/uber|lyft|gas|fuel|parking|transit/))
  ...
  if (lower.match(/electric|water|internet|phone|bill/))
