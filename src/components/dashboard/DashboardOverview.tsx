...
export function DashboardOverview() {
  ...
  const periodIncome = useMemo(
    () => sumFromDataPoints(filteredData, "income"),
    [filteredData],
  );
  const periodExpenses = useMemo(
    () => sumFromDataPoints(filteredData, "expenses"),
    [filteredData],
  );
  const savingsRate = useMemo(
    () => calculateSavingsRate(periodIncome, periodExpenses),
  ...
  const budgetUsage = useMemo(
    () =>
      calculateBudgetUsage(data?.summary.totalExpenses || 0, estimatedBudget),
    [data?.summary.totalExpenses, estimatedBudget],
  );
      ...
      >
          ...
          <div className="flex gap-10 mt-8 text-sm font-medium text-neutral-500">
            {stats.map((stat, i) => (
              <div key={i} className="flex flex-col group cursor-default">
                <span className="text-neutral-400 text-xs uppercase tracking-wider group-hover:text-neutral-600 transition-colors">
                  {stat.label}
                </span>
                <div className="flex items-baseline gap-2 mt-2">
                  <span className="text-3xl text-neutral-900 group-hover:scale-105 transition-transform origin-left">
                    {stat.value}
                  </span>
                </div>
              </div>
        ...
        <div className="flex gap-4">
          <Button
            asChild
            variant="outline"
            className="h-12 px-6 rounded-full border-neutral-200 hover:bg-neutral-50 hover:text-neutral-900 text-neutral-600 gap-2 text-base"
          >
            <Link href="/budgets">
          ...
          <Button
            asChild
            className="h-12 px-6 rounded-full bg-neutral-900 text-white hover:bg-neutral-800 gap-2 shadow-lg shadow-neutral-900/20 text-base"
          >
      ...
      {showAiTip && (
        ...
        >
          ...
          <div className="flex-1">
            <h4 className="font-medium text-indigo-900 text-base">
              💡 AI Insight
            </h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 h-auto">
            ...
            <Link href="/settings">
              <Card className="h-full bg-gradient-to-br from-[#E8E6E1] to-[#DCD6CC] border-none shadow-soft relative overflow-hidden p-8 flex flex-col justify-end group cursor-pointer rounded-[32px]">
                ...
                <div className="relative z-10 text-white transform transition-transform duration-300 group-hover:translate-y-[-4px]">
                  <h3 className="text-4xl font-medium tracking-tight mb-1">
                    Demo User
                  </h3>
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <Progress
                        value={65}
                        className="h-2 mt-4 bg-neutral-100"
        ...
        <div className="md:col-span-8 flex flex-col gap-8 h-full">
              ...
              <div className="flex items-center gap-6 mt-8 pt-6 border-t border-neutral-100">
                ...
                <div>
                  ...
                  <div className="text-sm text-neutral-400">
                    {budgetUsage < 80
                      ? "Nice restraint! You're on track 🎯"
                      : budgetUsage < 100
                        ? "Getting close - watch those impulse buys!"
                        : "Over budget - let's rein it in"}
                  </div>
                </div>
                <div className="ml-auto flex gap-4">
                  <div className="text-right">
                    ...
                    <div
                      className={`font-medium text-lg ${projectedSavings > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {projectedSavings > 0 ? "💰 +" : ""}
                      {formatCurrency(projectedSavings)} this month
                    </div>
                  </div>
                </div>
              </div>
          ...
          <motion.div
            variants={itemVariants}
            whileHover={hoverScale}
            className="h-1/3 grid grid-cols-2 gap-8"
          >
            <Link href="/recurring">
      ...
      <motion.div variants={itemVariants} className="pt-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/transactions">
            <Button
              variant="ghost"
              className="hover:bg-neutral-100 text-base px-6 h-12"
            >
              View All
            </Button>
          </Link>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-neutral-50 text-neutral-500 font-medium border-b border-neutral-100">
                <tr>
                  <th scope="col" className="px-8 py-5 text-base font-normal">
                    Transaction
                  </th>
                  <th scope="col" className="px-8 py-5 text-base font-normal">
                    Category
                  ...
                  <th
                    scope="col"
                    className="px-8 py-5 text-right text-base font-normal"
                  >
                    Amount
                  </th>
                  <th
                    scope="col"
                    className="px-8 py-5 text-center text-base font-normal"
                  >
                    Status
                  </th>
                </tr>
                  ...
                  (data?.recentTransactions || []).slice(0, 5).map((t, i) => (
                    ...
                    >
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center ${t.amount < 0 ? "bg-red-50 text-red-500" : "bg-green-50 text-green-500"}`}
                          >
                            {t.amount < 0 ? (
                          ...
                          <span className="font-medium text-base text-neutral-900">
                            {t.description || "Unknown Transaction"}
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                      ...
                      <td
                        className={`px-8 py-5 text-right font-medium text-base ${t.amount > 0 ? "text-green-600" : "text-neutral-900"}`}
                      >
