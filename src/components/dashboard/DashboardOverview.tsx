...
export function DashboardOverview() {
    ...
    // Mock data for visual parity with reference image
    const stats = [
        { label: "Transactions", value: data?.summary.transactionCount || 0, trend: "+12%" },
        { label: "Savings Rate", value: "24%", trend: "+5%" },
        { label: "Net Worth", value: formatCurrency(data?.summary.totalBalance || 0), trend: "+8%" },
    ];

    // Mock Chart Data if empty
    const chartData = (data?.dailyTrend && data.dailyTrend.length > 0) ? data.dailyTrend : [
        ...
        >
            ...
            <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-4">
                ...
                <div className="flex gap-4">
                    <Button variant="outline" className="h-12 px-6 rounded-full border-neutral-200 hover:bg-neutral-50 hover:text-neutral-900 text-neutral-600 gap-2 text-base">
                        <Wallet className="w-5 h-5" />
                        Add Budget
                    </Button>
                    <Button className="h-12 px-6 rounded-full bg-neutral-900 text-white hover:bg-neutral-800 gap-2 shadow-lg shadow-neutral-900/20 text-base">
            ...
            {showAiTip && (
                ...
                >
                    ...
                    <button onClick={() => setShowAiTip(false)} className="text-indigo-400 hover:text-indigo-600 p-2 hover:bg-indigo-100 rounded-full transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </motion.div>
            )}
                        ...
                        <Card className="h-full bg-white border-none shadow-soft p-8 relative flex flex-col rounded-[32px]">
                            <div className="flex justify-between items-center mb-8">
                                ...
                                <div className="flex gap-3">
                                    <Button variant="ghost" size="sm" className="text-sm text-neutral-500 hover:text-neutral-900 px-4">Weekly</Button>
                                    <Button variant="ghost" size="sm" className="text-sm bg-neutral-100 text-neutral-900 px-4">Monthly</Button>
                                </div>
                            </div>

                            <div className="flex-1 w-full min-h-0">
                                <SpendingLineChart data={chartData} isLoading={isLoading} />
                            </div>
                            ...
                            <div className="flex items-center gap-6 mt-8 pt-6 border-t border-neutral-100">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm text-neutral-400">You are on track</div>
                                </div>
                                <div className="ml-auto flex gap-4">
                                    <div className="text-right">
                                        <div className="text-sm text-neutral-400">Projected Savings</div>
                                        <div className="font-medium text-lg text-green-600">+$1,240.00</div>
                                    </div>
                                </div>
                            </div>
                        </Card>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-neutral-50 text-neutral-500 font-medium border-b border-neutral-100">
                                <tr>
                                    <th className="px-8 py-5 text-base font-normal">Transaction</th>
                                    <th className="px-8 py-5 text-base font-normal">Category</th>
                                    <th className="px-8 py-5 text-base font-normal">Date</th>
                                    <th className="px-8 py-5 text-right text-base font-normal">Amount</th>
                                    <th className="px-8 py-5 text-center text-base font-normal">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-neutral-100">
                                ...
                                {(data?.recentTransactions || []).length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-8 py-16 text-center text-neutral-400 text-base">
