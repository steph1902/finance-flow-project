...
export async function POST(request: Request) {
    try {
        ...
        for (let month = 0; month < 3; month++) {
            transactions.push(
                { userId: demoUser.id, amount: 2150, type: 'EXPENSE' as const, category: 'Housing', description: 'Monthly Rent', date: new Date(new Date().getFullYear(), new Date().getMonth() - month, 1) },
                { userId: demoUser.id, amount: 120, type: 'EXPENSE' as const, category: 'Utilities', description: 'Electric Bill', date: new Date(new Date().getFullYear(), new Date().getMonth() - month, 8) },
                { userId: demoUser.id, amount: 80, type: 'EXPENSE' as const, category: 'Utilities', description: 'Internet', date: new Date(new Date().getFullYear(), new Date().getMonth() - month, 5) },
                { userId: demoUser.id, amount: 142, type: 'EXPENSE' as const, category: 'Transportation', description: 'Car Insurance', date: new Date(new Date().getFullYear(), new Date().getMonth() - month, 22) },
            );
            ...
            for (let week = 0; week < 4; week++) {
                transactions.push({
                    ...
                    date: new Date(new Date().getFullYear(), new Date().getMonth() - month, 7 * week + 3),
                });
            }
            ...
            for (let i = 0; i < 8; i++) {
                transactions.push({
                    ...
                    type: 'EXPENSE' as const,
                    category: 'Dining',
                    description: ['Chipotle', 'Starbucks', 'Panera Bread', 'Local Cafe'][i % 4],
                    date: new Date(new Date().getFullYear(), new Date().getMonth() - month, Math.floor(Math.random() * 28) + 1),
                });
            }
        }
        ...
        const goals = [
            ...
            { name: 'Vacation Fund', target: 8000, current: 3200, category: 'Entertainment' },
        ];
        ...
        const notifications = [
            ...
            { type: 'SYSTEM', title: 'Welcome to FinanceFlow', message: 'Your demo account is set up!' },
        ];
