...
export async function POST(request: NextRequest) {
  try {
    ...
    const exportData = await exportUserData({
      ...
      ...(data.includeTransactions !== undefined && { includeTransactions: data.includeTransactions }),
      ...(data.includeBudgets !== undefined && { includeBudgets: data.includeBudgets }),
      ...(data.includeGoals !== undefined && { includeGoals: data.includeGoals }),
      ...(data.includeRecurring !== undefined && { includeRecurring: data.includeRecurring }),
      ...(data.includeInvestments !== undefined && { includeInvestments: data.includeInvestments }),
    });
    ...
    headers.set('Content-Disposition', `attachment; filename="${filename}.${extensions[data.format]}"`);
