...
export default function AIQualityPage() {
  ...
  const fetchMetrics = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/ai-quality?days=${selectedDays}${variantParam}`,
      );
  ...
  const triggerAggregation = async () => {
    setLoading(true);
    try {
      const { runAggregationAction } =
        await import("@/app/[locale]/(dashboard)/admin/demo-data/actions");
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Historical trends over {selectedDays} days
          </CardDescription>
        </CardHeader>
        <CardContent>
...
function MetricCard({
  ...
  const isBetter =
    diff !== null &&
    ((lowerIsBetter && diff < 0) || (!lowerIsBetter && diff > 0));
