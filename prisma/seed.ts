...
async function main() {
  ...
  for (let month = 0; month < 12; month++) {
    ...
    transactions.push({
      userId: demoUser.id,
      amount: randomAmount(
        utilityProviders.electricity.range[0],
        utilityProviders.electricity.range[1],
      ),
      type: "EXPENSE",
      category: "Utilities",
    ...
    transactions.push({
      userId: demoUser.id,
      amount: randomAmount(
        utilityProviders.gas.range[0],
        utilityProviders.gas.range[1],
      ),
      type: "EXPENSE",
      category: "Utilities",
    ...
    transactions.push({
      userId: demoUser.id,
      amount: randomAmount(
        utilityProviders.water.range[0],
        utilityProviders.water.range[1],
      ),
      type: "EXPENSE",
      category: "Utilities",
  ...
  for (let i = 0; i < transactions.length; i += batchSize) {
    ...
    console.log(
      `   Inserted ${Math.min(i + batchSize, transactions.length)}/${transactions.length}`,
    );
  }
  ...
  const notifications = [
    ...
    },
    {
      ...
      message: "Your rent payment of $2,150 is due tomorrow.",
    },
    {
      type: "SYSTEM",
      title: "Welcome to FinanceFlow",
      message:
        "Your account is set up. Start by connecting your bank accounts!",
    },
  ];
