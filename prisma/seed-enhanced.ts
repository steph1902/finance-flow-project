    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (dayOfWeek >= 1 && dayOfWeek <= 5 && Math.random() > 0.6) {
        transactions.push({
          ...
          date: new Date(
            currentDate.setHours(12, Math.floor(Math.random() * 60)),
          ),
        });
      }
    ...
    for (let i = 0; i < entertainmentCount; i++) {
      transactions.push({
        ...
        description: randomItem([
          ...MERCHANTS.Entertainment.slice(6, 8),
          "PlayStation Store",
          "Concert Tickets",
        ]),
        date: new Date(
...
async function main() {
  ...
  const notifications = [
    ...
    },
    {
      ...
      message:
        "Your demo account is ready. Explore budgets, goals, and AI-powered insights.",
      priority: 0,
    },
  ];
