...
**Cost Equation**:
$$ C\_{txn} = (1500 \times \$0.35 \times 10^{-6}) + (50 \times \$1.05 \times 10^{-6}) \approx \$0.0006 $$
...
2.  **Warm Hit**: On subsequent requests, we pass only the _delta_ transaction $t_{\text{new}}$ and the reference pointer to $K$.
...
$$ \pi(s) = \underset{a}{\text{argmax}} \sum\_{t=0}^{\infty} \gamma^t R(s_t, a_t) $$
...
$$ | Total*{receipt} - \sum*{i \in I} price_i | > \epsilon $$
...
1.  **Sutton, R. S., & Barto, A. G.** (2018). _Reinforcement Learning: An Introduction_. (MDP Formalism).
2.  **Pearl, J.** (1988). _Probabilistic Reasoning in Intelligent Systems_. (Bayesian Networks).
3.  **Google DeepMind.** (2024). _Gemini 1.5 Technical Report_.
4.  **Fowler, M.** (2006). _Patterns of Enterprise Application Architecture_.
