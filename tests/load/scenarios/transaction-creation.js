...
export function handleSummary(data) {
  return {
    ...
    "tests/load/results/transaction-creation-summary.json":
      JSON.stringify(data),
...
function textSummary(data, options) {
  ...
  summary +=
    indent + `  Failed Requests: ${(httpFailed.rate * 100).toFixed(2)}%\n\n`;
  ...
  summary +=
    indent + `  Status: ${thresholdsPassed ? "✅ PASSED" : "❌ FAILED"}\n`;
  summary +=
    indent + `  p95 < 200ms: ${httpDuration["p(95)"] < 200 ? "✅" : "❌"}\n`;
  summary +=
    indent + `  p99 < 500ms: ${httpDuration["p(99)"] < 500 ? "✅" : "❌"}\n`;
  summary +=
    indent + `  Error rate < 1%: ${errors.rate < 0.01 ? "✅" : "❌"}\n\n`;
